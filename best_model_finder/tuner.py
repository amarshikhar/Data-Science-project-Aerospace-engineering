...
class Model_Finder:
    ...
    def get_best_params_for_random_forest(self,train_x,train_y):
        ...
        try:
            ...
            self.logger_object.log(self.file_object, "Getting best parameter for model with Randomized search.")
            grid = RandomizedSearchCV(estimator=self.rf_model, param_distributions=param_grid, cv=5,  verbose=3,n_jobs=-1)
    ...
    def get_best_params_for_xgboost(self,train_x,train_y):
        ...
        try:
            ...
            self.logger_object.log(self.file_object, "Getting best parameter for model with Randomized search.")
            grid= RandomizedSearchCV(self.xgb_model,param_grid_xgboost, verbose=3,cv=5,n_jobs=-1)
            # finding the best parameters
            grid.fit(train_x, train_y)
            self.logger_object.log(self.file_object, "RandomizedSearchCV search for parameters complete.")
